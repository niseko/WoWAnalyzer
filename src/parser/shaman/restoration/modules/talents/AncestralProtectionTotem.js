...
//tfCxVRQANXz9LD7K/57-Mythic+G'huun+-+Kill+(8:10)/
...
class AncestralProtectionTotem extends Analyzer {
  ...
  constructor(...args) {
    ...
    // disable if there are multiple shaman, these events have no way of figuring out who its from
    ...
    //if (restoShamans && restoShamans.some(shaman => shaman.hasTalent(SPELLS.ANCESTRAL_PROTECTION_TOTEM_TALENT.id))) {
  ...
  statistic() {
    console.log(this.lifeSavingEvents);
                      ...
                      <tr key={index}>
                        ...
                        <td style={{ textAlign: 'center' }}>
                          <SpellLink id={event.ability.guid} icon={false}>
                            <Icon icon={event.ability.abilityIcon} /> {event.type}
                          </SpellLink></td>
                      </tr>
                    );
                  }
